...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const { selectedBuilding, buildings, setSelectedBuilding } = useApp();
...
export function AppSidebar() {
  ...
  if (isMobile) {
    return (
      <>
        ...
        <header className="fixed top-0 left-0 right-0 z-40 bg-background border-b border-border px-4 py-3 lg:hidden">
          <div className="flex items-center gap-3">
            ...
            <img src={mpsLogo} alt="MPS Logo" className="h-8 w-auto" />
            <span className="font-semibold">BMAI</span>
          </div>
        </header>
  ...
  return (
    <aside className="hidden lg:flex h-screen w-[280px] flex-col border-r border-sidebar-border">
      <SidebarContent />
    </aside>
  );
}
